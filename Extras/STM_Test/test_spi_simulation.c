...
uint16_t spi_rx_buffer[RX_LEN] = {
    0x3F9D, 0x70A4, 0xBF7A, 0xE148,  // ab_p
    0x3F0F, 0x5C29, 0x3FB9, 0x999A,  // hip_p
    0xBF8F, 0x5C29, 0x3F47, 0xAE14,  // knee_p
    0x400F, 0xC215, 0xBFC7, 0xAE14,  // ab_v
    0x3F8F, 0x5C29, 0x3F7A, 0xE148,  // hip_v
    0xBEE6, 0x6666, 0x3FC7, 0xAE14,  // knee_v
    0x42C8, 0x0000, 0x4316, 0x0000,  // ab_kp, hip_kp
    0x4348, 0x0000, 0x437A, 0x0000,  // knee_kp
    0x42F0, 0x0000, 0x432F, 0x0000,  // ab_kd, hip_kd
    0x3E99, 0x999A, 0x3ECC, 0xCCCC,  // knee_kd
    0x3F00, 0x0000, 0x3F19, 0x999A,  // ab_t
    0x3F33, 0x3333, 0x3F4C, 0xCCCC,  // hip_t
    0x3E4C, 0xCCCC, 0xBE99, 0x999A,  // knee_t
    0xBECC, 0xCCCC, 0x3F00, 0x0000,  // flags
    0x3F19, 0x999A, 0xBF33, 0x3333,  // flags
    0x40C6  // Checksum (last value)
};
...
// Convert NaN to uint16_t and insert correctly
void insert_nan_to_spi_buffer()
{
    float nan_value = NAN;
    uint16_t nan_bytes[2];

    // Convert NaN into 16-bit chunks
    memcpy(nan_bytes, &nan_value, sizeof(nan_value));

    // Insert NaN into the buffer (position 30, 31 for flags[1])
    spi_rx_buffer[0] = nan_bytes[0];
    spi_rx_buffer[1] = nan_bytes[1];

    printf("Inserted NaN at spi_rx_buffer[30] and spi_rx_buffer[31]\n");
}
...
int check_nan_in_spi_rx(spi_rx *data)
{
    ...
    printf("\n=== SPI Structure Debug Info ===\n");
    printf("Size of spi_rx structure: %lu bytes\n", sizeof(spi_rx));
    printf("Size of float: %lu bytes\n", sizeof(float));
    printf("Size of flags: %lu bytes\n", sizeof(data->flags));
    printf("Size of checksum: %lu bytes\n", sizeof(data->checksum));
...
control.knee_t[CAN]  = 0.0f;
 ...
 // Flags
control.flags[0] = 0x00020001;
control.flags[1] = 0x00010001;
// Checksum
control.checksum = 0x3000;
	printf("\n=== SPI Test Start ===\n");

	// Load Old Control Values (Before Overwriting)

	// Print Old Control Values
	printf("Old Control Values (Before Overwriting):\n");

  insert_nan_to_spi_buffer();
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef * hspi)
{
	...
	uint8_t validData = 1;
	printf("\n=== HAL_SPI_TxRxCpltCallback Triggered ===\n");

	printf("Received SPI Buffer (Raw Data):\n");
    for(int i = 0; i < RX_LEN; i++)
    {
        printf("spi_rx_buffer[%d] = 0x%X\n", i, spi_rx_buffer[i]);
    }
	...
	printf("Copied SPI Data to valuesrec Structure\n");
	...
	printf("Extracted Control Mode: %u\n", currentControlMode);
	printf("Flags After Masking: flags[0] = 0x%X, flags[1] = 0x%X\n", valuesrec.flags[0], valuesrec.flags[1]);
		...
		if(validData == 1)
		{
			if(valuesrec.checksum == checksum_calc && (valuesrec.flags[0]<=3 || valuesrec.flags[1]<=3))
			{
				...
				printf("Control Structure Updated Successfully.\n");
			}
		}
		else
		{			
